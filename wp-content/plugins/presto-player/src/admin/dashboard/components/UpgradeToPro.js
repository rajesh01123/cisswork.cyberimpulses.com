...
const UpgradeToPro = ( { layout = 'vertical' } ) => {
	const features = [
		...
		__( 'AI Analytics Abilities', 'presto-player' ),
		__( 'Email capture', 'presto-player' ),
	];

	const history = useHistory();
	const { label, onClick, isPremium } = useUpgradeCTA( history );
			...
			<Container className="gap-3" direction="column">
				...
				<Container className="gap-1" direction="column">
					<Title
						tag="h2"
						title={ __(
							'Power up your videos with Presto Player Pro',
							'presto-player'
						) }
						size="sm"
